...
async def create_refresh_token(user_id: str, db: AsyncSession) -> str:
    ...
    # Revoke old tokens for this user (single-session policy)
...
async def rotate_refresh_token(raw_token: str, db: AsyncSession) -> tuple[str, str]:
    """
    Validate existing refresh token, revoke it, issue new access + refresh pair.
...
async def revoke_all_tokens(user_id: str, db: AsyncSession):
    """Revoke all refresh tokens — called on suspend."""
    result = await db.execute(
        select(RefreshToken).where(
            RefreshToken.user_id == uuid.UUID(user_id),
            RefreshToken.is_revoked == False,
