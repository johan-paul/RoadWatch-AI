...
class CitizenRegisterRequest(BaseModel):
    ...
    firebase_id_token: str  # from Firebase after OTP verified on device
...
class FirebaseLoginRequest(BaseModel):
    """Existing users (citizen or officer) log in by sending their Firebase ID token."""
...
class ComplaintCreate(BaseModel):
    ...
    # image_url set after upload to Supabase Storage
...
class HeatmapPoint(BaseModel):
    lat: float
    lng: float
    weight: float   # severity-weighted intensity
    complaint_id: str
    status: ComplaintStatus
