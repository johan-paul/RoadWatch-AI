...
async def check_phone(
    ...
    """
    Step 1 of login — mobile app calls this before triggering Firebase OTP.
    Returns the account status so the app knows what screen to show next.
    """
...
async def login(
    ...
    Mobile sends Firebase ID token (after OTP verified on device).
    We verify it, look up the user, and return our JWT pair.
    """
    try:
...
async def register_citizen(
    ...
    Mobile sends Firebase ID token (OTP already verified on device).
    We verify the token, confirm phone matches, create the account.
    Officers CANNOT use this endpoint — their accounts are created by admin.
    """
    try:
        firebase_data = await verify_firebase_token(body.firebase_id_token)
    except ValueError as e:
    ...
    # Confirm the phone in the token matches what the user submitted
    ...
    db.add(AuditLog(
        ...
        target_id=str(user.id),
        ip_address=_client_ip(request),
        metadata={"name": user.name, "phone": user.phone_number},
    ))
...
async def refresh_token(
    ...
    # Decode access token to get metadata for response
